...
function ArticleManager(props) {
        ...
        render: (articleId, record) => {
          return (
            <ul className='action-list'>
              ...
              >
                <Button onClick={e => e.preventDefault()} type='link' size='small'>
                  更多操作
                </Button>
              </Dropdown>
            </ul>
  ...
  const rowSelection = batch ? { selectedRowKeys, onChange: selectList => setSelectedRowKeys(selectList) } : null
    ...
    <div className='admin-article-manager'>
      ...
      <Form layout='inline' onFinish={handleSubmit} style={{ marginBottom: 20 }}>
        ...
        <Form.Item label='标签' name='tag'>
          <Select style={{ width: 200 }} allowClear placeholder='请选择标签'>
            {tagList.map(item => (
        ...
        <Form.Item label='分类' name='category'>
          <Select style={{ width: 200 }} allowClear placeholder='请选择分类'>
        ...
        <Form.Item label='私密性' name='type'>
          <Select style={{ width: 200 }} allowClear placeholder='请选择私密性'>
